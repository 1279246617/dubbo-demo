...
public class MybatisServicePlugin extends PluginAdapter {
	...
	protected void addServiceImpl(TopLevelClass topLevelClass,IntrospectedTable introspectedTable, String tableName, List<GeneratedJavaFile> files) {
		...
		topLevelClass.addMethod(setCriteria(introspectedTable, tableName));
	...
	//设置查询条件方法
	private Method setCriteria(IntrospectedTable introspectedTable,
			String tableName) {
		Method method = new Method();
		...
		List<IntrospectedColumn> introspectedColumnsList=introspectedTable.getAllColumns();
		 for (IntrospectedColumn introspectedColumn : introspectedColumnsList) {
			 if(introspectedColumn.isStringColumn()){
				 ...
				 String conditionProperty=javaProperty.substring(0, 1).toUpperCase() + javaProperty.substring(1);
				 method.addBodyLine("if(StringUtils.isNotEmpty(params.getOrDefault(\""+javaProperty+"\", \"\").toString()))");
				 method.addBodyLine("\tcriteria.and"+conditionProperty+"EqualTo(params.getOrDefault(\""+javaProperty+"\", \"\").toString());");
			 }
         }
		return method;
	}
