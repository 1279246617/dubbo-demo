...
addCustomsPresetting = function(){
    $.dialog({
    		...
    		width: 360,
    		height: 240,
    		content: 'url:'+ctxpath+'/order.CustomsPresettings/addCustomsPresetting',
    		button: [{
    			name: '确认',
    			callback: function() {
                    var that = this;
...
//修改
updateCustomsPresetting = function(skuCustomsId){
    $.dialog({
    		lock: true,
    		max: false,
    		min: false,
    		...
    		width: 360,
    		height: 240,
    		content: 'url:'+ctxpath+'/order.CustomsPresettings/updateCustomsPresetting?skuCustomsId='+skuCustomsId,
    		button: [{
    			name: '确认',
    			callback: function() {
                    var that = this;
...
//删除 单条数据 
deleteCustomsPresetting = function(skuCustomsId){
    if(!skuCustomsId){
    ...
    $.dialog({
    		...
    		button: [{
    			name: '确认',
    			callback: function() {
    				$.getJSON(ctxpath+"/order.CustomsPresettings/deleteCustomsPresetting?skuCustomsId="+skuCustomsId,function(msg) {
    					grid.loadData()
    				});
    			}
    		},
...
// 批量删除
deleteBatchCustomsPresetting = function(){
    		...
    		button: [{
    			...
    			callback: function() {
    				for ( var i = 0; i < row.length; i++) {
                		$.post(ctxpath+'/order.CustomsPresettings/deleteCustomsPresetting',{skuCustomsId: row[i].id},function(msg){
